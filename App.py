...
CITY_REF_FILE      = "City for Aoutomation.xlsx"
...
def get_urls_from_sheet(sheet_name: str) -> list[str]:
    """Return all cleaned URLs from a specific DB sheet (case-insensitive name match)."""
    df = _load_db_sheet(sheet_name)
...
def validate_and_prepare_urls(
    ...
    """
    Check each user URL against the selected DB sheet only.
    - Matched  → use as-is (validated)
    - Not found → append to that sheet, still use it
    Returns (all_urls_to_use, newly_appended_urls)
    """
    ...
    db_urls_set = set(get_urls_from_sheet(sheet_name))

    validated   = [u for u in cleaned_user if u in db_urls_set]
    not_found   = [u for u in cleaned_user if u not in db_urls_set]

    # Append new URLs to the DB sheet so they persist
    if not_found:
        append_urls_to_sheet(sheet_name, not_found)

    # All user URLs are valid to use (matched + newly appended)
...
def load_city_db_sheet(sheet_name: str) -> list[tuple[str, float]]:
    """
    Load (city_name, weight) pairs from the given sheet in City for Automation.xlsx.
    Weight = Potential Impressions. Returns sorted by weight descending.
    """
    try:
        df = pd.read_excel(CITY_REF_FILE, sheet_name=sheet_name)
        ...
        df = df[df["_city"] != ""]
        df["_weight"] = df[weight_col].apply(safe_float) if weight_col else 1.0
        df = df[df["_weight"] > 0].sort_values("_weight", ascending=False)
        return [(row["_city"], float(row["_weight"])) for _, row in df.iterrows()]
    except Exception as e:
        print(f"[WARN] load_city_db_sheet('{sheet_name}') failed: {e}")
        return []
...
def build_sheet8_city(df1: pd.DataFrame, df2: pd.DataFrame,
    ...
    if li_col1 and li_col2:
        items1 = {_normalize_text(v) for v in df1[li_col1].dropna().unique() if str(v).strip()}
        items1.discard("")
        ...
        mask = df2[li_col2].apply(_matches)
    ...
    # ── Phase 2: City Column Detection in File 2 ──────────────────────────────
    city_col = None
    ...
    info["debug"]["city_col"] = city_col

    # ── Phase 3: Load City DB Sheet ───────────────────────────────────────────
    db_cities = load_city_db_sheet(city_db_sheet)
    if not db_cities:
        db_cities = load_city_db_sheet("Master Database")
        if db_cities:
            info["warnings"].append(
                f"Sheet '{city_db_sheet}' not found in City DB. Using Master Database."
            )

    # Build a normalized lookup: lower_name → (original_name, weight)
    db_lookup = {c.strip().lower(): (c, w) for c, w in db_cities}

    # ── Phase 4: Determine Final City List ────────────────────────────────────
    if city_col is not None:
        # Extract unique cities from File 2 pivot
        pivot_cities_raw = (
            matched_df2[city_col]
            .dropna().astype(str).str.strip()
            .unique().tolist()
        )
        pivot_cities_lower = {
            c.lower() for c in pivot_cities_raw
            if c.lower() not in ("", "nan", "none", "unknown")
        }

        # Match pivot cities against selected DB sheet
        matched_from_db = [
            db_lookup[c] for c in pivot_cities_lower if c in db_lookup
        ]

        if matched_from_db:
            final_cities = matched_from_db
            info["city_source"] = f"pivot_matched_db ({city_db_sheet})"
            info["debug"]["pivot_cities"] = list(pivot_cities_lower)[:10]
            info["debug"]["matched_count"] = len(matched_from_db)
        else:
            # No matches → use all DB sheet cities (capped by impression range)
            info["warnings"].append(
                f"No cities from File 2 matched '{city_db_sheet}'. "
                "Using all cities from the selected DB sheet."
            )
            info["city_source"] = f"db_no_match ({city_db_sheet})"
            if total_imp < 50_000:
                cap = random.randint(15, 30)
            elif total_imp < 200_000:
                cap = random.randint(25, 50)
            else:
                cap = random.randint(40, 70)
            # Shuffle top cities to add variety, then cap
            top = db_cities[:min(150, len(db_cities))]
            random.shuffle(top)
            final_cities = top[:cap]
    else:
        # No city column → use all DB sheet cities
        info["warnings"].append(
            f"No 'City' column in File 2. Using cities from DB sheet '{city_db_sheet}'."
        )
        info["city_source"] = f"db_no_city_col ({city_db_sheet})"
        if total_imp < 50_000:
            cap = random.randint(15, 30)
        elif total_imp < 200_000:
            cap = random.randint(25, 50)
    ...
    city_names   = [c[0] for c in final_cities]
    city_weights = [max(c[1], 1.0) for c in final_cities]
    w_total      = sum(city_weights)

    # ── Phase 5: Distribute Impressions ──────────────────────────────────────
    city_imps = _largest_remainder(city_weights, w_total, total_imp)
    city_imps = deduplicate_preserving_sum(city_imps, gap=5)

    # ── Phase 6: Distribute Clicks (CTR 0.35%–0.56%) ─────────────────────────
    city_clks = _distribute_clicks(city_imps)

    # ── Phase 7: Build & Return ───────────────────────────────────────────────
    rows = _build_rows(city_names, city_imps, city_clks)
    info["cities_found"] = len(rows)

    return rows, grand_total, info
...
def build_sheet10_apps(
    ...
    db_sheet_urls = [u for u in get_urls_from_sheet(selected_sheet)
...
def process_file(job_id: str, filepath1: Path, filepath2: Path | None,
    ...
    try:
        ...
        s8 = s8t = s9 = s9t = None
        if df2 is not None:
            s8, s8t, city_info     = build_sheet8_city(df1, df2, total_imp, total_clk, ctr_reach, city_sheet)
...
def generate_html_report(job_id: str, sheets_data: dict) -> str:
    ...
    logo_path = Path(__file__).resolve().parent / "New update" / "BILLION TAGS PNG white.png"
    logo_data = ""
    if logo_path.exists():
        try:
